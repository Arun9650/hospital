...
*   **Chat — mounted + read receipts + pagination (Phase 3 pull-forward, 2026-07-15)** — the async chat (built but previously unmounted) is now reachable at `/patient/messages` and `/doctor/messages` with sidebar nav links. Added **read receipts** (`read_at` column, `markThreadRead`, `read` Realtime broadcast, sent/read tick indicators) and **message pagination** (`recent_chat_messages` window RPC caps the initial load to the most-recent 50 per thread; a "Load earlier" pager fetches older pages via `loadOlderChatMessages` and preserves scroll) — `0017_chat_read_pagination.sql`, `lib/db.ts`, `lib/actions/data.ts`, `components/ChatClient.tsx`. See §4.1.3.
*   **Chat — file/image attachments (Phase 2, 2026-07-16)** — a private `chat-attachments` bucket + participant-scoped storage RLS, `attachment_path/name/type` on `chat_messages`, `sendChatAttachment` (15 MB cap) with signed-URL reads, a paperclip composer control, and inline image / file-chip rendering (`0018_chat_attachments.sql`, `lib/db.ts`, `lib/actions/data.ts`, `components/ChatClient.tsx`). See §4.1.3.

*   **Medical records — file storage (Phase 2)** — patients can upload lab reports/scans/etc. (Supabase Storage `medical-records` bucket, user-id-scoped upload paths) from the records page and attach documents during booking; records list gained real view/download links (`0010_medical_records_files.sql`, `lib/actions/records.ts`, `components/UploadRecordButton.tsx`). See §4.1.5 for remaining gaps (license uploads, doctor-side access).
*   **Role-aware read RLS (Phase 2)** — replaced the demo-permissive "world-readable" policies with participant/role-scoped SELECT across appointments, prescriptions, medical_records, notifications, chat, and profiles, keyed off `profiles.role` via security-definer helpers; PHI readers moved to the auth-aware server client; `medical-records` flipped to a **private** bucket served via signed URLs (`0011_role_aware_rls.sql`, `lib/db.ts`). See §5.2.
*   **Write authorization (Phase 2)** — split the clinical-table policies into per-command INSERT/UPDATE/DELETE so a patient can't self-issue prescriptions or write another patient's records: appointments created via the booking RPC / by the patient, prescriptions issued only by doctors, medical_records uploaded only by the owner (`0012_write_authorization.sql`).
*   **Notification write hardening (Phase 2)** — closed the last permissive write: `notifications` INSERT is now denied to the client and every runtime insert routes through a security-definer `create_notification` RPC that authorizes the caller↔target relationship (doctor→their patient, patient→their booked doctor, self, admin), so spoofed cross-user notifications via the REST API are blocked (`0013_notify_rpc.sql`, `notify()` in `lib/actions/data.ts`). Booking already inserted its doctor notification via `book_appointment` (0007). See §5.2.
*   **Chat & verification write scoping (Phase 2)** — `chat_messages` can now only be inserted into a thread the caller participates in, with `sender` bound to the caller's role in that thread (blocks cross-thread injection and doctor/patient impersonation); `chat_threads` are patient-created; `verification_queue` is admin-only for read and write (0011 had left its read world-open). `getVerificationQueue` moved to the auth-aware client so the admin portal reads real rows under the tightened RLS (`0014_chat_verification_writes.sql`, `lib/db.ts`). See §5.2.
*   **Doctor-side medical records (Phase 2)** — a treating doctor now sees a patient's uploaded documents (with signed-URL View links) in the patient-record detail pane, scoped by RLS to their own patients via `doctor_sees_patient()` on both the `medical_records` rows and the private storage objects (`0015_doctor_records_access.sql`, `getPatientMedicalRecords`/`loadPatientRecords`, `components/DoctorPatientsClient.tsx`). See §4.1.5 / §5.2.
*   **Audit logging (Phase 3 pull-forward)** — critical clinical/admin actions now append to a tamper-resistant `audit_log`: a security-definer `log_audit` RPC stamps the actor from the session, the table denies direct client INSERT, and reads are admin-only. Wired into prescription issuance (both paths), verification decisions, appointment status changes, and record uploads, and surfaced in a paginated **admin audit viewer** at `/admin/audit` (`0016_audit_log.sql`, `lib/audit.ts`, `getAuditLog`, `components/AuditLogClient.tsx`, call sites in `lib/actions/data.ts` + `lib/actions/records.ts`). Closes the tractable half of the last §5.2 item; at-rest encryption remains. See §5.2.
*   **Server-side doctor search & pagination (Phase 3 pull-forward)** — doctor discovery moved off the client: `searchDoctors` queries Supabase with text/specialty/mode/fee/rating filters, sort, and `range()` pagination (`count: 'exact'`); the `/patient/doctors` page SSRs page 1 and `DoctorSearchClient` refetches on filter change (debounced, stale-guarded) with a **Load more** pager. Untrusted query fields are clamped in `searchDoctorsAction` and the `.or()` text term is sanitized against filter-injection (`lib/db.ts`, `lib/actions/data.ts`, `components/DoctorSearchClient.tsx`). Addresses §4.1.5 (search), §5.1 & §5.3 (pagination). See §4.1.5.
...
*   Payments, social login, real LLM assistant, admin content management, real analytics, paid TURN. (Role-aware RLS, file storage, and pagination / server-side search have since shipped — see §5.2, §4.1.5; rate limiting foundation shipped — see §5.2.)
...
**Implementation status (updated 2026-07-14):** Booking wizard (mode → slot → details → payment), booking-slot double-booking dedup, doctor accept/decline, and realtime request delivery already shipped. Newly shipped: **patient cancel** (`cancelMyAppointment`) and **reschedule** (`rescheduleAppointment`) from the appointments dashboard with doctor notification; **calendar invite (.ics) download** and **booking reference number** on confirmation (`lib/ics.ts`); and a **fee-acknowledgement gate** before payment. Rate limiting on booking now shipped (10/hour per account via `check_rate_limit`; see §5.2). Still open: real payment gateway, "Pay at Consultation", waiting list, auto-approve, no-show marking, timezone conversion, automatic reminders, Zod migration, and server-side paginated doctor search/filters.
...
**Implementation status (updated 2026-07-14):** Realtime bidirectional messaging (Supabase Realtime postgres_changes), left/right message bubbles, per-message timestamps, unread counts/badges, optimistic send with echo dedupe, and the mock-mode canned-reply demo already shipped. Newly shipped: **auto-scroll to newest message** on send/receive/thread-switch; **typing indicators** (Realtime broadcast on the shared channel in live mode, simulated during the mock reply); **doctor quick-reply templates** (one tap fills the composer); and **conversation search** over loaded threads (`components/ChatClient.tsx`). **Updated 2026-07-15:** the async chat is now **mounted on real routes** — `/patient/messages` and `/doctor/messages` (server pages + sidebar "Messages" nav links) render `ChatClient` (it was previously built but unreferenced). **Read receipts** shipped: a `read_at` column (`0017_chat_read_pagination.sql`), a `markThreadRead` action that stamps the other party's messages read on open/receive, live propagation via a Realtime `read` broadcast, and single-tick (sent) / double-tick (read) indicators on your own bubbles. **Message pagination** shipped: `getChatThreads` now loads only the most-recent 50 per thread via a one-round-trip `recent_chat_messages` window RPC, with a **Load earlier messages** pager (`loadOlderChatMessages`) that prepends older pages and preserves scroll position (§5.3). **Updated 2026-07-16:** **file/image attachments** shipped — a private `chat-attachments` Supabase Storage bucket with participant-scoped storage RLS (path `<threadId>/…`, mirrors `chat_messages` 0014), `attachment_path/name/type` columns on `chat_messages`, a `sendChatAttachment` upload action (15 MB cap), signed-URL reads minted per load (batch on history, on-demand for realtime arrivals via `signChatAttachmentUrl`), a paperclip composer button + hidden file input, and inline image thumbnails / file-download chips in bubbles (`0018_chat_attachments.sql`, `lib/db.ts`, `lib/actions/data.ts`, `components/ChatClient.tsx`). Still open: threading/replies, soft delete + retention, mute, offline queue, at-rest PHI encryption, email notifications, auto-scroll-triggered infinite scroll (currently a button), and a "Message this doctor" entry point to start new threads (RLS is participant-scoped since 0011/0014).
...
*   **Medical Records Management:**
    ...
    *   **Implementation status (updated 2026-07-14):** Patients can **upload** records (title + type + file) from the records page and **attach documents during booking**, stored in a `medical-records` Supabase Storage bucket (uploads namespaced by user id; insert policy scopes each user to their own folder) with a `medical_records` row; the records list now offers real **view/download** links (`0010_medical_records_files.sql`, `lib/actions/records.ts`, `components/UploadRecordButton.tsx`). The bucket was since made **private** with signed-URL reads (0011). **Doctor-side access** is now done: a treating doctor sees a patient's uploaded documents in the patient-record detail pane, gated by RLS to their own patients (`0015_doctor_records_access.sql`, `getPatientMedicalRecords`/`loadPatientRecords`, `components/DoctorPatientsClient.tsx`). Still open: doctor license-document uploads.
*   **Doctor Search & Filter:**
    ...
    *   **Implementation status (updated 2026-07-15):** **Done.** Doctor discovery now runs server-side: `searchDoctors` (`lib/db.ts`) queries the `doctors` table with `ilike` name/specialty text search, `specialty`/`modes`/`fee`/`rating` filters, sort, and `range()`-based pagination (`count: 'exact'`), exposed to the client via the input-clamping `searchDoctorsAction` (`lib/actions/data.ts`). The page SSRs the first page (honouring `?q`/`?specialty`), and `DoctorSearchClient` refetches on filter change (debounced, stale-response-guarded) with a **Load more** pager. Mock mode and hard query errors fall back to an in-memory filter that mirrors the SQL exactly; the `.or()` text term is sanitized against PostgREST filter-injection (§5.2). Still open: geographical-location and insurance filters (no such columns/data yet), and tag substring matching server-side (kept client-parity in the mock fallback only).
*   **AI Health Assistant:**
...
*   The system shall implement robust, role-aware Row Level Security (RLS) policies to ensure data isolation and prevent unauthorized access to Protected Health Information (PHI).
    *   **Implementation status (updated 2026-07-14):** **Read isolation** (`0011_role_aware_rls.sql`) **and write authorization** (`0012_write_authorization.sql`) **shipped.** Appointments, prescriptions, medical_records, notifications, chat threads/messages, and profiles are participant/role-scoped for reads: a patient sees only their own rows (plus seeded NULL-owner demo rows), a doctor sees rows tied to them (incl. their patients' profile fields for the panel), an admin sees all — keyed off `profiles.role` (trigger-set, not user-editable JWT metadata) via security-definer helpers. Clinical **writes** are now per-command scoped: appointments are created only via the security-definer booking RPC / by the patient, prescriptions only by doctors, medical_records only by the owning patient. `notifications` INSERT is denied to the client — every runtime insert goes through the security-definer `create_notification` RPC (0013), which authorizes the caller↔target relationship (doctor→their patient, patient→their booked doctor, self, admin), so cross-user notifications can't be spoofed. PHI readers use the auth-aware server client, and `medical-records` is a **private** bucket read via short-lived signed URLs. `chat_*` and `verification_queue` writes are now scoped too (`0014_chat_verification_writes.sql`): a message can only be inserted into a thread the caller participates in, with its `sender` bound to the caller's role in that thread (no cross-thread injection or doctor/patient impersonation); `verification_queue` is admin-only for both read and write. Doctor-side read of a treated patient's medical_records (rows + private-bucket files) is now allowed via `doctor_sees_patient()` (`0015_doctor_records_access.sql`). **Audit logging** now records critical clinical/admin actions to an append-only `audit_log` table via a security-definer `log_audit` RPC that stamps the actor server-side; the table denies direct client writes and is admin-only for reads, surfaced in an **admin audit viewer** (`/admin/audit`, paginated with Load more) (`0016_audit_log.sql`, `lib/audit.ts`, `getAuditLog`, `components/AuditLogClient.tsx`). Audit coverage now spans booking, chat send, patient cancel/reschedule, and profile edits in addition to the clinical/admin paths above (2026-07-15). **Still open:** at-rest encryption of PHI.
...
*   The system shall implement rate limiting for API endpoints to prevent abuse and denial-of-service attacks.
    *   **Implementation status (updated 2026-07-16):** Foundation shipped — a fixed-window counter (`rate_limit` table + tamper-resistant `check_rate_limit` security-definer RPC that keys on the session actor and denies all direct client access) enforced from the write Server Actions via `lib/rateLimit.ts` `allowAction` (fail-open on infra error): **booking** 10/hour (§4.1.1), **chat send** 30/min, **chat attachment** 20/hour (`0019_rate_limit.sql`, `lib/actions/data.ts`). Remaining: cover the rest of the write paths (cancel/reschedule, prescriptions, uploads, profile) and unauthenticated auth endpoints (sign-in/up/reset — key by email/IP; Supabase Auth also applies its own limits); fixed-window can briefly allow ~2× at a boundary (swap to sliding window if needed); a periodic sweep of stale windows (currently opportunistic per-key).
...
*   The system shall maintain comprehensive audit logs for all critical user and administrative actions.
    *   **Implementation status (updated 2026-07-15):** Foundation shipped — an append-only `audit_log` table + tamper-resistant security-definer `log_audit` RPC (actor stamped from the session; direct client INSERT denied; admin-only read), wired into prescription issuance, verification decisions, appointment status changes, and medical-record uploads, and surfaced in an admin viewer at `/admin/audit` (`0016_audit_log.sql`, `lib/audit.ts`, `components/AuditLogClient.tsx`). **Updated 2026-07-15:** audit coverage now extends to the remaining write paths — booking (`appointment.book`), chat send (`chat.message`), patient cancel/reschedule (`appointment.cancel`/`appointment.reschedule`), and profile edits (`profile.update`) — via `logAudit` in `lib/actions/data.ts` + `lib/actions/profile.ts`. Remaining: at-rest PHI encryption (the heavier §5.2 item).
