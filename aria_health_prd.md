...
*   **Audit logging (Phase 3 pull-forward)** — critical clinical/admin actions now append to a tamper-resistant `audit_log`: a security-definer `log_audit` RPC stamps the actor from the session, the table denies direct client INSERT, and reads are admin-only. Wired into prescription issuance (both paths), verification decisions, appointment status changes, and record uploads (`0016_audit_log.sql`, `lib/audit.ts`, call sites in `lib/actions/data.ts` + `lib/actions/records.ts`). Closes the tractable half of the last §5.2 item; at-rest encryption remains. See §5.2.
...
*   The system shall maintain comprehensive audit logs for all critical user and administrative actions.
    *   **Implementation status (updated 2026-07-15):** Foundation shipped — an append-only `audit_log` table + tamper-resistant security-definer `log_audit` RPC (actor stamped from the session; direct client INSERT denied; admin-only read), wired into prescription issuance, verification decisions, appointment status changes, and medical-record uploads (`0016_audit_log.sql`, `lib/audit.ts`). Remaining: cover the rest of the write paths (chat, booking, profile edits, cancel/reschedule) and add an admin viewer.
