...
*   The system shall implement robust, role-aware Row Level Security (RLS) policies to ensure data isolation and prevent unauthorized access to Protected Health Information (PHI).
    *   **Implementation status (updated 2026-07-14):** **Read isolation shipped** (`0011_role_aware_rls.sql`). Appointments, prescriptions, medical_records, notifications, chat threads/messages, and profiles are now participant/role-scoped: a patient sees only their own rows (plus seeded NULL-owner demo rows), a doctor sees rows tied to them (incl. their patients' profile fields for the panel), an admin sees all — keyed off `profiles.role` (trigger-set, not user-editable JWT metadata) via security-definer helpers. The PHI readers were moved to the auth-aware server client so RLS runs as the signed-in user, and `medical-records` is now a **private** bucket read via short-lived signed URLs. **Deferred:** tightening WRITES (insert/update checks stay permissive because a doctor's action legitimately writes the patient's notification and booking uses a security-definer RPC); doctor-side access to medical_records; scoping `verification_queue`; at-rest encryption/audit logging.
